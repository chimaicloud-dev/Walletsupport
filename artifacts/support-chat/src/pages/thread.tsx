...
export default function ThreadPage() {
  ...
  useEffect(() => {
    if (!messages) return;
    const guestMessages = messages.filter((m) => m.senderType === "guest");
    ...
    if (prevMsgCountRef.current !== null && count > prevMsgCountRef.current) {
      ...
      showBrowserNotification(
        "New message",
        `${conversation?.guestName ?? "Visitor"} sent a message.`
      );
    }
      ...
      <div className="flex flex-col h-full relative">
        <header className="h-auto min-h-16 px-4 sm:px-8 py-4 flex flex-col sm:flex-row sm:items-center justify-between border-b border-border bg-card/50 backdrop-blur-sm sticky top-0 z-10 gap-4">
          <div className="flex items-start sm:items-center gap-4">
            <Link
              href="/inbox"
              className="p-2 -ml-2 rounded-md hover:bg-muted text-muted-foreground hover:text-foreground transition-colors mt-0.5 sm:mt-0"
            >
          ...
          <button
            className="text-xs text-muted-foreground hover:text-foreground flex items-center gap-1.5 shrink-0 ml-auto"
