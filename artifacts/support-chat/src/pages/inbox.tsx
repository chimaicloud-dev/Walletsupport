...
import { MessageSquare, User, Bell, BellOff } from "lucide-react";
import { Skeleton } from "@/components/ui/skeleton";
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import { playPing, showBrowserNotification, requestNotificationPermission } from "@/utils/notifications";
...
export default function InboxPage() {
  ...
  useEffect(() => {
    ...
    if (prevCountRef.current !== null && count > prevCountRef.current) {
      playPing();
      showBrowserNotification("New conversation", "Someone just started a new support chat.");
    } else if (prevUnreadRef.current !== null && unread > prevUnreadRef.current) {
      playPing();
      showBrowserNotification("New message", "You have a new unread message.");
    }
  ...
  return (
    <Layout>
      <div className="flex flex-col h-full">
        <header className="h-16 px-8 flex items-center border-b border-border bg-card shrink-0 gap-4">
          <h1 className="text-xl font-semibold text-foreground">Inbox</h1>
          {stats && (
            <div className="flex items-center space-x-4 text-sm">
              <span className="text-muted-foreground">
                <strong className="text-foreground font-medium">{stats.open}</strong> open
              </span>
              <span className="text-muted-foreground">
                <strong className="text-foreground font-medium">{stats.closed}</strong> closed
              </span>
            </div>
          )}
                    ...
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          {conv.status === "closed" && (
                            <Badge variant="secondary" className="text-xs font-normal h-5 shrink-0">
                              Closed
                            </Badge>
                          )}
                        </div>
