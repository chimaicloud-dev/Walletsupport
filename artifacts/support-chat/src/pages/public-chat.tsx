...
export default function PublicChatPage() {
  ...
  const tokenKey = `chat_token_handle_${handle}`;

  const { data: profile } = useGetUserByHandle(handle, {
    query: { queryKey: getGetUserByHandleQueryKey(handle), retry: false },
  });
  ...
  useEffect(() => {
    ...
    if (existing) {
      setLocation(`/chat/${handle}/thread/${existing}`);
    }
  }, [handle]);
